...
class Encoder(nn.Module):
    def __init__(self, hidden_size, height, width, bgru, step, flip):
        ...
        if self.step is not None:
            ...
            self.output_proj = nn.Linear(self.height//32*512*self.step, self.height//32*512)
        ...
        if self.bi: #8: 3 MaxPool->2**3    128: last hidden_size of layer4
            self.rnn = RNN(self.height//32*512, self.hidden_size, self.n_layers, dropout=self.dropout, bidirectional=True)
            if SUM_UP:
                self.enc_out_merge = lambda x: x[:,:,:x.shape[-1]//2] + x[:,:,x.shape[-1]//2:]
                self.enc_hidden_merge = lambda x: (x[0] + x[1]).unsqueeze(0)
        else:
            self.rnn = RNN(self.height//32*512, self.hidden_size, self.n_layers, dropout=self.dropout, bidirectional=False)
    ...
    def forward(self, in_data, in_data_len, hidden=None):
        ...
        out = out.view(-1, batch_size, self.height//32*512)
        if self.step is not None:
